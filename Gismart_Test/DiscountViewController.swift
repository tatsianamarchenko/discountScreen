...
class DiscountViewController: UIViewController{
	...
	@objc func showPopupScreen() {
		timerManager?.dateToTransfer { [weak self] daysLeft ,houseLeft, minutesLeft, secondsLeft  in
			let popup = PopupScreenViewController()
			popup.setMessage(message: "Offer activated at \(daysLeft)\(houseLeft)\(minutesLeft)\(secondsLeft)")
	...
	override func viewDidLoad() {
		super.viewDidLoad()
		timerManager = TimerManager(timerValue: 86400, timerView: timer)
